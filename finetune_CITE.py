...
if __name__ == "__main__":
    ...
    print(str(model))
    args.ae_weights = './results/pretrain/cite_76.pth.tar'
    t0 = time()
    ...
    ema_model = scDCC(input_dim=adata.n_vars, z_dim=32, n_clusters=args.n_clusters,
                     encodeLayer=[256, 64], decodeLayer=[64, 256], sigma=sd, gamma=args.gamma).cuda()
    for param in ema_model.parameters():
        param.detach_()
    global global_step
    global_step = 0
    # update_ema_variables(model, ema_model, alpha=0.999, global_step=global_step)
