...
def normalize(adata, filter_min_counts=None, size_factors=True, normalize_input=True, logtrans_input=True,
              highly_genes=None):
    if filter_min_counts != None:
        # sc.pp.filter_genes(adata, min_counts=1)
        sc.pp.filter_cells(adata, min_counts=1000)
        y = adata.X[:, -1]
    ori_adata = adata.copy()
    if size_factors:
    ...
    if highly_genes != None:
        ...
        # sc.pl.highest_expr_genes(adata,n_top=2000)
        adata1 = adata[:, adata.var.highly_variable]
    else:
        adata1 = adata
    if highly_genes != None:
        if size_factors or normalize_input or logtrans_input:
            adata1.raw = ori_adata[:, adata.var.highly_variable].copy()
        else:
            adata1.raw = ori_adata[:, adata.var.highly_variable]
    else:
        if size_factors or normalize_input or logtrans_input:
            adata1.raw = ori_adata.copy()
        else:
            adata1.raw = ori_adata
    # adata1.raw = adata.raw[:, adata.var.highly_variable]
    if normalize_input:
        sc.pp.scale(adata)
